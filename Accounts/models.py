...
class UserManager(BaseUserManager):
    ...
    def create_superuser(self, email, password=None, **extra_fields):
        ...
        # اگر بخوای نقش owner به‌صورت پیش‌فرض ست بشه:
        if 'role' not in extra_fields:
            try:
                owner_role = Role.objects.get(name='owner')
                extra_fields['role'] = owner_role
            except Role.DoesNotExist:
                pass  # اگر نقش owner هنوز ساخته نشده، خطا نده
...
class UserProposalUpload(models.Model):
    ...
    font_path = models.CharField(
        max_length=500,
        blank=True,
        null=True,
        help_text="مسیر کامل فونت برای خروجی PDF یا Word"
    )
    ...
    is_paid = models.BooleanField(default=False)  # فقط برای فایل‌های خریداری‌شده
    is_free = models.BooleanField(default=False)  # برای فایل‌های رایگان سایت
    is_editable = models.BooleanField(default=True)  # کنترل ویرایش
    is_downloadable = models.BooleanField(default=True)  # کنترل دانلود

    def can_user_edit(self, user):
        return (
                self.user == user or
                self.is_free or
                (self.is_paid and self.is_editable)
        )
    ...
    def can_user_download(self, user):
        return (
                self.user == user or
                self.is_free or
                (self.is_paid and self.is_downloadable)
        )
