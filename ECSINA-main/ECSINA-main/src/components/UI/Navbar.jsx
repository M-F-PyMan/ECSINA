...
const navigationLinks = [
  { id: 1, title: "صفحه اصلی", href: "/" },
  {
    ...
    href: "/products",
    subLink: [
      { title: "بوم کسب و کار", icon: "/assets/icons/Computer.svg", id: 1 },
      { title: "بیزنس مدل", icon: "/assets/icons/Status1.svg", id: 2 },
      { title: "بیزنس پلن", icon: "/assets/icons/Status2.svg", id: 3 },
      { title: "پروپوزال ها", icon: "/assets/icons/Document.svg", id: 4 },
      { title: "بوم ناب", icon: "/assets/icons/Status3.svg", id: 5 },
    ],
  },
        ...
        <div className="flex items-center gap-3 xl:gap-8">
          ...
          <ul className="flex items-center gap-2 lg:gap-8">
            {navigationLinks.map((link) => (
              <li key={`nav-${link.id}`} className="relative group">
                <Link
                ...
                {link.subLink && (
                  <ul className="absolute text-xl top-full opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 hidden group-hover:block bg-white shadow-dropdown rounded-xl p-5 px-3 w-[550px] h-72 mt-2 z-50 transition-all duration-300">
                    ...
                    {link.subLink.map((item) => (
                      <li key={`sub-${link.id}-${item.id}`} className="mb-2">
                        <Link
